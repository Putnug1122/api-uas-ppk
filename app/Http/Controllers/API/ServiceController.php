    ...
    public function getServiceList(Request $request){
        if(auth()->user() !== null){
            if(auth()->user()->hasRole('admin')){
                $service = Service::where('added_by',1)->withTrashed()->with(['providers','category','serviceRating']);
            }else{
                $service = Service::where('added_by',1)->withTrashed()->with(['providers','category','serviceRating']);
            }
        }else{
            $service = Service::where('added_by',1)->with(['providers','category','serviceRating']);
        }
